...
> Status original: **DECIDIDO** — Nomenclatura definida, gaps mapeados

> Nota de atualização
>
> Este documento registra a decisão arquitetural que levou ao layout atual, mas partes do texto abaixo descrevem o estado anterior do repositório durante a migração.
>
> Layout atual consolidado:
>
> - `backend/` → projeto Django/backend
> - `frontends/` → aplicações frontend
> - `docs/` → documentação interna do projeto
> - `frontends/docusaurus/docs/` → documentação navegável do fluxo de captura
...
```
backend/       → Django backend consolidado
  src/         → apps, config, core, infrastructure
  templates/   → templates Django
  static/      → output dos builds Vite
  media/       → media de runtime/upload
frontends/     → parent directory dos frontends React
